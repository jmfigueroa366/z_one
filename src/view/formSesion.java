...
import css.CancionTimelinePanel;
import model.Sesion;
...
public class formSesion extends JPanel {
    ...
    private CancionTimelinePanel timelineCancion;  // ← UNA sola instancia
    ...
    // ── HELPERS DE CATÁLOGO ─────────────────────────────────────────
    private String nombreArtista(Integer id)  { return id != null ? mapaArtistas.getOrDefault(id,    "Artista #"   + id) : "-"; }
    private String nombreProductor(Integer id){ return id != null ? mapaProductores.getOrDefault(id, "Productor #" + id) : "-"; }
    private String nombreCabina(Integer id)   { return id != null ? mapaCabinas.getOrDefault(id,     "Cabina #"    + id) : "-"; }
    private String nombreEstado(Integer id)   { return id != null ? mapaEstados.getOrDefault(id,     "Estado #"    + id) : "-"; }
    private String nombreFase(Integer id)     { return id != null ? mapaFases.getOrDefault(id,       "Fase #"      + id) : "-"; }
    private String nombreCancion(Integer id)  { return id != null ? mapaCanciones.getOrDefault(id,   "Canción #"   + id) : "-"; }

    private double duracion(Sesion s) {
        if (s.getHoraInicio() == null || s.getHoraFin() == null) return 0;
        return Math.max(0, ChronoUnit.MINUTES.between(s.getHoraInicio(), s.getHoraFin()) / 60.0);
    }
    private double costoEstimado(Sesion s) { return 0.0; }
    ...
    private JPanel headerPanel() {
        ...
        busqueda = new ModernUI.RoundedTextField("Buscar sesión...") {
            @Override protected void paintComponent(Graphics g) {
                ...
                if (hasFocus()) { g2.setColor(new Color(26, 167, 224, 210)); g2.setStroke(new BasicStroke(2f)); }
                else            { g2.setColor(new Color(0xD1D5DB));          g2.setStroke(new BasicStroke(1f)); }
                g2.drawRoundRect(1, 1, getWidth() - 2, getHeight() - 2, 12, 12);
                if (getText().isEmpty() && !hasFocus()) {
                    g2.setColor(new Color(0x9CA3AF));
                    g2.setFont(getFont());
                    FontMetrics fm = g2.getFontMetrics();
                    g2.drawString("Buscar sesión...", 16, (getHeight() + fm.getAscent() - fm.getDescent()) / 2);
                }
                g2.dispose();
                getUI().paint(g, this);
            }
        ...
        der.add(busqueda); der.add(btnVista); der.add(bGrabar); der.add(bRefr); der.add(bNueva);
    ...
    private void abrirGrabacion() {
        ...
        dialogGrabacion dlg = new dialogGrabacion(owner,
                seleccionada.getIdGrabacion(), seleccionada.getNombreSesion());
    ...
    // ── TABLA CARD — timeline SOLO aquí, ancho completo abajo ───────
    private JPanel tablaCard() {
        ...
        // ── Timeline SOLO en el SOUTH de tablaCard ───────────────────
        JPanel timelineWrapper = new JPanel(new BorderLayout());
        timelineWrapper.setOpaque(false);
        timelineWrapper.setBorder(new EmptyBorder(10, 0, 0, 0));

        JLabel tTimeline = new JLabel("ESTADO DE CANCIÓN");
        tTimeline.setFont(new Font("Segoe UI", Font.BOLD, 10));
        tTimeline.setForeground(C_ACCENT_CYAN);
        tTimeline.setBorder(new EmptyBorder(0, 2, 6, 0));
        timelineWrapper.add(tTimeline, BorderLayout.NORTH);

        timelineCancion = new CancionTimelinePanel();
        timelineCancion.setNombreCancion("Selecciona una sesión");
        timelineCancion.setFaseActual(0);
        timelineWrapper.add(timelineCancion, BorderLayout.CENTER);

        card.add(timelineWrapper, BorderLayout.SOUTH);
    ...
    private JComponent filaSesion(Sesion s) {
        ...
        String artNombre = s.getNombreArtista() != null ? s.getNombreArtista() : nombreArtista(s.getIdArtista());
        ...
        String prodNombre = s.getNombreProductor() != null ? s.getNombreProductor() : nombreProductor(s.getIdProductor());
        ...
        JLabel hor = new JLabel(horaStr(s));
    ...
    private String horaStr(Sesion s) {
        ...
        return hi + "-" + s.getHoraFin().toLocalTime().toString().substring(0, 5);
    }
    ...
    private JPanel pillEstado(Integer idEstado) {
        ...
        pill.add(dot); pill.add(txt);
        return pill;
    }

    private Color colorEstadoById(Integer id) {
        if (id == null) return new Color(0xF3F4F6);
        return switch (id) { case 1 -> new Color(0xEFF6FF); case 2 -> new Color(0xFFFBEB);
                             case 3 -> new Color(0xF0FDF4); case 4 -> new Color(0xFEF2F2);
                             default -> new Color(0xF3F4F6); };
    }
    private Color colorEstadoFgById(Integer id) {
        if (id == null) return new Color(0x6B7280);
        return switch (id) { case 1 -> new Color(0x1D4ED8); case 2 -> new Color(0x92400E);
                             case 3 -> new Color(0x166534); case 4 -> new Color(0x991B1B);
                             default -> new Color(0x6B7280); };
    }
    private Color colorEstadoAccentById(Integer id) {
        if (id == null) return C_BORDER;
        return switch (id) { case 1 -> C_PRIMARY; case 2 -> new Color(0xF59E0B);
                             case 3 -> C_OK;       case 4 -> C_ERR;
                             default -> C_BORDER; };
    }
    ...
    private void construirLista(List<Sesion> data) {
        ...
        if (data.isEmpty()) {
            ...
            vacio.setFont(FS); vacio.setForeground(C_TEXT_MUT);
    ...
    private JLabel filaTarjeta(String texto) {
        ...
        l.setFont(FE); l.setForeground(C_TEXT_MUT); l.setAlignmentX(LEFT_ALIGNMENT);
        return l;
    }

    // ── PANEL LATERAL — SIN timeline aquí ───────────────────────────
    ...
    private void actualizarGrabaciones() {
        ...
        if (seleccionada == null) {
            ...
            grabacionesBox.revalidate(); grabacionesBox.repaint();
            return;
        }
        try {
            ...
            if (lista.isEmpty()) {
                ...
                lbl.setFont(new Font("Segoe UI", Font.PLAIN, 10));
                lbl.setForeground(C_TEXT_MUT); lbl.setAlignmentX(LEFT_ALIGNMENT);
                grabacionesBox.add(lbl);
            } else {
                for (model.Grabacion g : lista) {
                    JPanel fila = new JPanel(new BorderLayout(6, 0));
                    fila.setOpaque(false); fila.setAlignmentX(LEFT_ALIGNMENT);
                    ...
                    nombre.setForeground(C_TEXT_PRI); nombre.setAlignmentX(LEFT_ALIGNMENT);
                    JLabel meta = new JLabel(g.getDuracionSegundos() + "s  ·  " + g.getTamanoKb() + " KB");
                    meta.setFont(new Font("Segoe UI", Font.PLAIN, 9));
                    meta.setForeground(C_TEXT_MUT); meta.setAlignmentX(LEFT_ALIGNMENT);
                    info.add(nombre); info.add(meta);
                    ...
                    acciones.setOpaque(false);
                    acciones.add(btnPlay); acciones.add(btnEliminar);
            ...
            err.setForeground(C_ERR); err.setAlignmentX(LEFT_ALIGNMENT);
    ...
    private JButton buildIconBtn(String icon, Color fg) {
        ...
        b.setForeground(fg); b.setOpaque(false); b.setContentAreaFilled(false);
    ...
    private void eliminarGrabacion(int idGrabacion) {
        int confirm = JOptionPane.showConfirmDialog(this, "¿Eliminar esta grabación?",
    ...
    private void reproducir(String ruta, JButton btn) {
        try {
            if (clipActual != null && clipActual.isRunning()) {
                clipActual.stop(); clipActual.close(); clipActual = null;
                btn.setText("▶"); return;
            }
            File archivo = new File(ruta);
            if (!archivo.exists()) { toast("Archivo no encontrado: " + ruta, MainFrame.ToastType.ERROR); return; }
            AudioInputStream stream = AudioSystem.getAudioInputStream(archivo);
            clipActual = AudioSystem.getClip();
            clipActual.open(stream); clipActual.start();
            btn.setText("⏹");
    ...
    private JComponent rankingFila(int pos, Sesion s) {
        ...
        n.setForeground(C_TEXT_PRI); n.setAlignmentX(LEFT_ALIGNMENT);
        String artNombre = s.getNombreArtista() != null ? s.getNombreArtista() : nombreArtista(s.getIdArtista());
        JLabel d = new JLabel(artNombre + " · " + nombreCabina(s.getIdCabina()));
        d.setFont(new Font("Segoe UI", Font.PLAIN, 9));
        d.setForeground(C_TEXT_MUT); d.setAlignmentX(LEFT_ALIGNMENT);
    ...
    private void actualizarPanelLateral() {
        ...
        orden.sort(Comparator.comparing(s -> s.getFechaGrabacion() != null ? s.getFechaGrabacion() : LocalDate.MAX));
        int pos = 1;
        for (Sesion s : orden) {
            if (pos > 3) break;
        ...
        rankingBox.revalidate(); rankingBox.repaint();
        ...
        String prod = sesiones.stream()
                .map(s -> { String n = s.getNombreProductor(); return (n != null && !n.isBlank()) ? n : nombreProductor(s.getIdProductor()); })
    ...
    private List<Sesion> filtrar() {
        ...
        return sesiones.stream().filter(s -> {
            ...
            String art  = s.getNombreArtista()  != null ? s.getNombreArtista().toLowerCase()  : nombreArtista(s.getIdArtista()).toLowerCase();
            String prod = s.getNombreProductor()!= null ? s.getNombreProductor().toLowerCase(): nombreProductor(s.getIdProductor()).toLowerCase();
    ...
    // ── TIMELINE — actualiza la única instancia en tablaCard ─────────
    private void actualizarTimeline() {
        if (timelineCancion == null) return;
        if (seleccionada == null) {
            timelineCancion.setFaseActual(0);
            timelineCancion.setNombreCancion("Selecciona una sesión");
            return;
        }
        Integer idFase = seleccionada.getIdFase();
        timelineCancion.setFaseActual(idFase != null ? idFase : 1);
        timelineCancion.setNombreCancion(nombreCancion(seleccionada.getIdCancion()));
    }
    ...
    private void openForm(Sesion se) {
        ...
        JPanel root = new JPanel(new BorderLayout()) {
            @Override protected void paintComponent(Graphics g) {
                Graphics2D g2 = (Graphics2D) g.create();
                g2.setColor(C_BG_DARK); g2.fillRect(0, 0, getWidth(), getHeight()); g2.dispose();
            }
        };
        ...
        FieldFx fNombre    = new FieldFx(isEdit && se.getNombreSesion()    != null ? se.getNombreSesion()    : "", "Nombre de la sesión", timersDlg);
        FieldFx fFecha     = new FieldFx(isEdit && se.getFechaGrabacion()  != null ? se.getFechaGrabacion().format(FMT) : "", "dd/MM/yyyy", timersDlg);
        String hiStr = (isEdit && se.getHoraInicio() != null) ? se.getHoraInicio().toLocalTime().toString().substring(0, 5) : "09:00";
        String hfStr = (isEdit && se.getHoraFin()    != null) ? se.getHoraFin().toLocalTime().toString().substring(0, 5)    : "12:00";
        FieldFx fHIni      = new FieldFx(hiStr, "HH:mm", timersDlg);
        FieldFx fHFin      = new FieldFx(hfStr, "HH:mm", timersDlg);
        FieldFx fNumSesion = new FieldFx(isEdit && se.getNumeroSesion() != null ? se.getNumeroSesion().toString() : "", "Número (opcional)", timersDlg);
        FieldFx fObs       = new FieldFx(isEdit && se.getNotas()        != null ? se.getNotas()                  : "", "Notas / observaciones", timersDlg);
        ...
        ComboFx<Integer> cbEstado  = SesionComponents.comboFxObj(idsEstados,     id -> mapaEstados.getOrDefault(id,     "Estado "    + id), timersDlg);
        ComboFx<Integer> cbCabina  = SesionComponents.comboFxObj(idsCabinas,     id -> mapaCabinas.getOrDefault(id,     "Cabina "    + id), timersDlg);
        ComboFx<Integer> cbFase    = SesionComponents.comboFxObj(idsFases,       id -> mapaFases.getOrDefault(id,       "Fase "      + id), timersDlg);
        ComboFx<Integer> cbCancion = SesionComponents.comboFxObj(idsCanciones,   id -> mapaCanciones.getOrDefault(id,   "Canción "   + id), timersDlg);
        ComboFx<Integer> cbArt     = SesionComponents.comboFxObj(idsArtistas,    id -> mapaArtistas.getOrDefault(id,    "Artista "   + id), timersDlg);
        ComboFx<Integer> cbProd    = SesionComponents.comboFxObj(idsProductores, id -> mapaProductores.getOrDefault(id, "Productor " + id), timersDlg);

        if (isEdit && se.getIdEstadoGrabacion() != null) cbEstado.setSelectedItem(se.getIdEstadoGrabacion());  else if (idsEstados.length     > 0) cbEstado.setSelectedIndex(0);
        if (isEdit && se.getIdCabina()          != null) cbCabina.setSelectedItem(se.getIdCabina());           else if (idsCabinas.length     > 0) cbCabina.setSelectedIndex(0);
        if (isEdit && se.getIdFase()            != null) cbFase.setSelectedItem(se.getIdFase());               else if (idsFases.length       > 0) cbFase.setSelectedIndex(0);
        if (isEdit && se.getIdCancion()         != null) cbCancion.setSelectedItem(se.getIdCancion());         else if (idsCanciones.length   > 0) cbCancion.setSelectedIndex(0);
        if (isEdit && se.getIdArtista()         != null) cbArt.setSelectedItem(se.getIdArtista());             else if (idsArtistas.length    > 0) cbArt.setSelectedIndex(0);
        if (isEdit && se.getIdProductor()       != null) cbProd.setSelectedItem(se.getIdProductor());          else if (idsProductores.length > 0) cbProd.setSelectedIndex(0);
        ...
        filasFx.add(filaDoble("N° SESIÓN",          fNumSesion, "FASE *",        cbFase));
        filasFx.add(seccionTituloFx("DETALLES", timersDlg));
        filasFx.add(filaDoble("ESTADO *",           cbEstado,   "CABINA *",      cbCabina));
        filasFx.add(filaDoble("ARTISTA *",          cbArt,      "PRODUCTOR *",   cbProd));
        filasFx.add(filaSimple("CANCIÓN *",         cbCancion));
        filasFx.add(filaSimple("NOTAS",             fObs));
        ...
        bSave.addActionListener(e -> {
            ...
            catch (DateTimeParseException ex) { fFecha.shake(); toast("Formato de fecha inválido (dd/MM/yyyy)", MainFrame.ToastType.ERROR); return; }

            LocalDateTime horaIni = null, horaFin = null;
            try {
                if (!hiT.isEmpty()) horaIni = LocalDateTime.of(fecha, java.time.LocalTime.parse(hiT));
                if (!hfT.isEmpty()) horaFin = LocalDateTime.of(fecha, java.time.LocalTime.parse(hfT));
            } catch (Exception ex) { toast("Formato de hora inválido (HH:mm)", MainFrame.ToastType.ERROR); return; }
            if (horaIni != null && horaFin != null && !horaFin.isAfter(horaIni)) {
                toast("La hora de fin debe ser posterior a la de inicio", MainFrame.ToastType.ERROR); return;
            }

            Integer numSesion = null;
            if (!numStr.isEmpty()) {
                try { numSesion = Integer.parseInt(numStr); }
                catch (NumberFormatException ex) { fNumSesion.shake(); toast("El número de sesión debe ser entero", MainFrame.ToastType.ERROR); return; }
            }

            if (isEdit) {
                se.setNombreSesion(nm); se.setFechaGrabacion(fecha);
                se.setHoraInicio(horaIni); se.setHoraFin(horaFin);
                se.setNumeroSesion(numSesion); se.setIdArtista(idArt);
                se.setIdProductor(idProd); se.setIdCabina(idCab);
                se.setIdFase(idFase); se.setIdCancion(idCancion);
                se.setIdEstadoGrabacion(idEstado); se.setNotas(obs.isEmpty() ? null : obs);
                if (actualizarEnServicio(se)) {
                    toast("Sesión actualizada correctamente", MainFrame.ToastType.SUCCESS);
                    aplicarFiltro(); cerrarConFade(dlg);
                }
            } else {
                Sesion nueva = new Sesion();
                nueva.setNombreSesion(nm); nueva.setFechaGrabacion(fecha);
                nueva.setHoraInicio(horaIni); nueva.setHoraFin(horaFin);
                nueva.setNumeroSesion(numSesion); nueva.setIdArtista(idArt);
                nueva.setIdProductor(idProd); nueva.setIdCabina(idCab);
                nueva.setIdFase(idFase); nueva.setIdCancion(idCancion);
                nueva.setIdEstadoGrabacion(idEstado); nueva.setNotas(obs.isEmpty() ? null : obs);
                ...
                if (idGenerado > 0) {
                    ...
                    sesiones.add(nueva); seleccionada = nueva;
        ...
        dlg.addWindowListener(new java.awt.event.WindowAdapter() {
            @Override public void windowClosed(java.awt.event.WindowEvent e) {
                for (Timer t : timersDlg) t.stop(); timersDlg.clear();
            }
        });
        abrirConFade(dlg);
    ...
    private void abrirConFade(JDialog dlg) {
        ...
        t.addActionListener(ev -> {
            ...
            try { dlg.setOpacity(1f - (float) Math.pow(1 - p, 3)); } catch (Exception ignore) {}
            if (p >= 1f) t.stop();
        });
    ...
    private JPanel filaDoble(String l1, JComponent c1, String l2, JComponent c2) {
        JPanel p = new JPanel(new GridLayout(1, 2, 16, 0)) {
            @Override protected void paintComponent(Graphics g) {
                ...
                if (a < 1f) ((Graphics2D) g).setComposite(java.awt.AlphaComposite.getInstance(java.awt.AlphaComposite.SRC_OVER, a));
                super.paintComponent(g);
            }
        };
        p.setOpaque(false); p.setAlignmentX(LEFT_ALIGNMENT);
        p.setMaximumSize(new Dimension(Integer.MAX_VALUE, 70));
        p.add(filaCampo(l1, c1)); p.add(filaCampo(l2, c2));
        return p;
    }
    ...
    private JComponent seccionTituloFx(String texto, List<Timer> timersDlg) {
        ...
        p.add(l, BorderLayout.WEST); p.add(linea, BorderLayout.CENTER);
        return p;
    }
