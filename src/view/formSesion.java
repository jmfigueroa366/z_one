...
private void abrirGrabacion() {
    ...
    dlg.setOnGrabacionGuardada(this::actualizarGrabaciones); // refresca en tiempo real
    dlg.setVisible(true);
    actualizarGrabaciones(); // refresca al cerrar también
}
        // ── KPI CARDS ───────────────────────────────────────────────────
        ...
        private JComponent filaSesion(Sesion s) {
            ...
            Color ac = colorEstado(s.getEstadoSesion());
            fila.add(SesionComponents.avatar(iniciales(s.getArtista().getNombreArtista()), ac), BorderLayout.WEST);
        ...
        private JComponent crearTarjeta(Sesion s) {
            ...
            idar.add(SesionComponents.avatar(iniciales(s.getArtista().getNombreArtista()), ac));
            JLabel nom = new JLabel(s.getNombreSesion());
...
card.add(grabacionesBox);
    ...
    btnPlay.setForeground(C_ACCENT_CYAN);
    btnPlay.setBackground(new Color(0x0A1F36));
    ...
    fila.add(btnPlay, BorderLayout.EAST);
...
btnEliminar.setForeground(new Color(0xEF4444));
btnEliminar.setBackground(new Color(0x0A1F36));
...
fila.add(new JLabel("🎵"), BorderLayout.WEST);
fila.add(info, BorderLayout.CENTER);
fila.add(acciones, BorderLayout.EAST); // <-- reemplaza el add anterior de btnPlay
}
        }
    } catch (Exception ex) {
    JLabel err = new JLabel("Error: " + ex.getMessage()); // <-- agrega ex.getMessage()
...
private void reproducir(String ruta, JButton btn) {
    try {
        // Si hay algo reproduciéndose, detenerlo
        ...
        // Cuando termina, resetear el botón
        ...
        private void openForm(Sesion se) {
            ...
            ComboFx<Artista>   cbArt  = SesionComponents.comboFxObj(artistas.toArray(new Artista[0]),
                    v -> v instanceof Artista ? ((Artista) v).getNombreArtista() : "", timersDlg);
            ComboFx<Productor> cbProd = SesionComponents.comboFxObj(productores.toArray(new Productor[0]),
                    v -> v instanceof Productor ? ((Productor) v).getNombre() : "", timersDlg);
